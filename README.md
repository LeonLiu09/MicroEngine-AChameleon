...
然后打开 `http://localhost:4173/`。首页可直接点击“体验 Daniel 演示账号”，完整体验发现、搜索、收藏、个人资料、技能交换请求、匹配中心和资料编辑。

## 功能亮点

- 中文默认界面与完整英文切换，选择会保存在浏览器中。
- 确定性双向技能匹配、同水平学习搭子和三人 SkillLoop。
- 粉色 Social Gallery 视觉系统，以及两行反向流动的技能标签。
- 桌面/平板顶部导航、移动端底部导航和减少动态效果支持。
- 纯前端 Hash 路由与 `localStorage` 持久化；“重置演示”只删除 SkillSwap 自己的数据。
- 内置测试页：`http://localhost:4173/?selftest=1`。
